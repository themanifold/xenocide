...
CItem * DEFINITIONS :: find_item(string name)
{
...
CMonster * DEFINITIONS :: find_monster(string name)
{
...
bool DEFINITIONS :: read_items_definitions()
{
  ...
  int read = 0;
   ...
   CAmmo *ammo=NULL;
   CHandWeapon *handw=NULL;
   CRangedWeapon *shooting=NULL;
   CArmor *armor=NULL;
   ...
   CPill *pill=NULL;
   CTrash *garbage=NULL;  
   CRepairKit *repairset=NULL;
   CCorpse *cialo=NULL;  
   CProcessor *processor=NULL;
   CRobotLeg *leg=NULL;
   CRobotShell *shell=NULL;
   CProgrammator *programmator=NULL;
   CBattery *battery=NULL;
  ...
  while (line!="")
  {
    ...
    if (line.find('[')!=-1) // new_one item
    {
      ...
      else if (line.find("[GRENADE]")!=-1)
      {
        ...
        new_one=(CItem *) grenade;
      }
      else if (line.find("[CONTROLLER]")!=-1)
      {
        controller=new CONTROLLER;
        new_one=(CItem *) controller;
      }
      ...
      else if (line.find("[COPYOF ")!=-1)
      {
        ...
        ammo=(CAmmo *)new_one;
        shooting=(CRangedWeapon *)new_one;
        handw=(CHandWeapon*)new_one;
        armor=(CArmor*)new_one;
        grenade=(CGrenade*)new_one;
        skin=(NO_ARMOR *)new_one;
		controller=(CONTROLLER *)new_one;
		pill = (CPill *)new_one;
		garbage = (CTrash *)new_one;
		repairset = (CRepairKit *)new_one;
		cialo = (CCorpse *)new_one;
		processor=(CProcessor *)new_one;
		leg = (CRobotLeg*)new_one;
		shell = (CRobotShell*)new_one;		
      }
      else
      {
      ...
      items.push_back(new_one);
                  ...
                  else if (line.find("PROPERTIES =")!=-1)
                  {
					 ...
					 {
						    new_one->activities="gdtwrm"; // without Load uNload							
					 }
...
bool DEFINITIONS :: read_monsters_definitions()
{
  ...
  while (line!="")
  {
    ...
    if (line.find('[')!=-1) // new_one monster
    {
      ...
      {
        zwierz=new CAnimal;
        new_one=(CMonster *) zwierz;
      }
      else if (line.find("[MADDOCTOR]")!=-1)
      {
		  doctor=new CMadDoctor;
		  new_one=(CMonster *) doctor;
		  ...
		  doctor->tail=*(CHandWeapon *)item;
		  
      }
      else if (line.find("[SEARCHLIGHT]")!=-1)
      {
		  searchlight=new CSearchLight;
		  new_one=(CMonster *) searchlight;
      }
	  else if (line.find("[WORM]")!=-1)
	  {
		  worm=new CWorm;
		  new_one=(CMonster *) worm;
	  }
      else if (line.find("[INTELLIGENT]")!=-1)
      {
        intel=new CIntelligent;
        new_one=(CMonster *) intel;
        intel->backpack.clear();
      }
      else if (line.find("[ROBOT]")!=-1)
      {
		  robot=new CRobot;
		  new_one=(CMonster *) robot;
		  robot->shell=NULL;
      }	  
      else if (line.find("[COPYOF ")!=-1)
      {
        ...
        zwierz=(CAnimal *)new_one;
        intel=(CIntelligent *)new_one;
        robot=(CRobot *)new_one;
      }
      else
      {
      ...
      monsters.push_back(new_one);
                  ...
                  {
					  ((CIntelligent *)new_one)->AI_restrictions = get_AI_restrictions(line,file.line_number);
                  }				  
                  else if (line.find("TILE =")!=-1)
                  {
                     line=get_from_quotations(real, file.line_number);
                  ...
                  else if (line.find("INVENTORY =")!=-1)
                  {
                     ...
                     if (item->IsCountable())
                     {
						  ...
						  CCountable *licz = item->IsCountable();
						  licz->quantity = value;						  
                          ((CIntelligent *)new_one)->pick_up_item(item,false);
                     ...
                     else
                     {
                        for (index=0;index<value;index++)
                        {
						  ...
						  {
                            ((CIntelligent *)new_one)->set_armor(item);
						  }
                          else if (real.find("AS WEAPON")!=-1)
                            ((CIntelligent *)new_one)->weapon = item;
                          else if (real.find("AS READY")!=-1)
							  ((CIntelligent *)new_one)->ready_weapon = item;
                        }
                     }
