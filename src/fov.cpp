...
void CMutualFOV::ConvertCornersToVisibleCells()
{
	int x,y;
...
bool CMutualFOV::CornerBlocks(const int &corner_x,const int &corner_y,const int &sgn_x,const int &sgn_y)
{
...
bool CMutualFOV::AnyCellBlocks(const CPosition &start,const CPosition &end)
{
...
void CMutualFOV::PropagateShadow(const CPosition &start, const CPosition &diff, const int &dist)
{
	CPosition new_pos = start;
	new_pos.x += diff.x;
	new_pos.y += diff.y;
...
void CMutualFOV::CheckLOS(const CPosition &start, const CPosition &end, const int &distance)
{
	CPosition start2;
...
void CMutualFOV::Start(CMap* map, unsigned int x, unsigned int y, int maxRadius)
{	
	m_map = map;
...
void CMutualFOV::CorrectNearest(const CPosition &start)
{
	int single_check[][4] = {
		-1,-1, -2,-2,
		1,1,	2,2,
