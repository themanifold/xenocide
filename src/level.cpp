...
bool CLevel :: entering_on_cell_possible(int x,int y)
{
...
int CLevel :: get_gas_density(int x, int y)
{
...
void CLevel :: clear_lists_to_delete()
{
   ptr_list::iterator m,_m;
   CItem *temp;
   CMonster *temp2;
...
void CLevel :: list_of_items_from_cell(ptr_list *items,int x,int y)
{
   ptr_list::iterator m,_m;
   CItem *item;
...
CMonster * CLevel :: monster_on_cell(int x,int y)
{
      ptr_list::iterator m,_m;
      CMonster *monster;
...
bool CLevel :: add_gas(int x,int y, PROPERTIES properties, int density)
{
   ptr_list::iterator m,_m;
   GAS *gas;
...
void CLevel::remove_monsters()
{
	ptr_list::iterator m,_m;
	for(m=monsters.begin(),_m=monsters.end(); m!=_m; m++)
	{
		CMonster *temp2=(CMonster *)*m;
		delete temp2;
	}
	monsters.clear();
}

void CLevel::FreeLevelMemory()
{
   ptr_list::iterator m,_m;
...
void CLevel::ChangeLevelTo(string ID_new_one)
{
	CToSave::LogLevelChange(ID_new_one);
	if (ID!="START")
	{
		...
		SaveLastUNToFile();
		player = CToSave::LoadPlayer(game.name_of_player);
	}
	...
	if (LoadObject()==false)
	{
		if (turn!=0)
		{
			...
			screen.console.zero();
			screen.console.add("Please wait. Creating level...",7);
			screen.console.show();
		}
		myrefresh();
	...
	else // level loaded
	{
			...
			screen.console.clean();
			screen.console.zero();
			screen.console.add("Please wait. Calculating changes...",7);
			screen.console.show();
			myrefresh();

			if (player->next_action_time > turn + 20000) // a loooot of time passed
			{
...
void CLevel::CreateLevel(string type)
{
	if (type=="Shuttle")
	{
...
void CLevel::CreateItemsOnLevel(string id)
{
...
void CLevel::CreateItemOnLevel(string id,int x,int y)
{
...
CItem *CLevel::CreateRandomItemFromCategory(int category, int x, int y, int parametr1, int parametr2)
{
...
void CLevel::CreateMonstersOnLevel(string id)
{
	if (levels.find(id)==levels.end())
	{
...
void CLevel::CreateMonsterOnLevel(string id,int x,int y, bool create_not_moving)
{
	map_of_probabilities::iterator m,_m;
	pair_of_probabilities p;
...
bool CLevel::find_empty_not_visible_place(int &x, int &y)
{
...
bool CLevel::find_empty_place(int &x, int &y)
{
...
void CLevel::add_to_items_on_map(CItem *item)
{
	if (item!=NULL)
	{
...
void CLevel::remove_from_items_on_map(CItem *item)
{
	if (item!=NULL)
	{
...
void CLevel::remove_last_item_on_map()
{
