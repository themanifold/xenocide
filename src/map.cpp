...
bool CMap :: close(int x, int y)
{
  ...
  name=cells[x][y].name;
  place=name.find("(opened)");
  
  if (place==-1)
    return false;
  to_find=name.substr(0,place) + "(closed)";
...
bool CMap :: open(int x, int y)
{
  ...
  name=cells[x][y].name;
  place=name.find("(closed)");
  
  if (place==-1)
    return false;
  to_find=name.substr(0,place) + "(opened)";
  found=definitions.find_terrain(to_find);
  if (found!=NULL)
  {
     cells[x][y].name=found->name;
...
STAIRS *CMap::StairsAt(int x,int y)
{
   STAIRS *stairs;
...
#define M_WALL "Metal Wall"
#define M_CLOSED_DOOR "Metal Door (closed)"

int CMap :: add_room(int x1,int y1, int x2, int y2, CELL *wall, CELL *door, float prawd_braku_sciany)
{
  int index;
...
void CMap :: CreateMapBuilding()
{
   ...
   predefined_door=definitions.find_terrain("Metal Door (opened)");
...
///////////////////////////////////// TO TESTOWE
	 // create stairs down
	 CELL *floor = definitions.find_terrain("Floor");
	 CELL *elevator = definitions.find_terrain("Stairs (down)");
	 ...
	 for (j = level.stairs_down[level.ID].begin(),_j = level.stairs_down[level.ID].end();j!=_j;j++)
	 {
		 ...
		 while(1)
		 {
			 ...
			 if (cells[x][y]==*floor)
			 {
				 cells[x][y]=*elevator;
				 stairs->to_where = *j;
				 stairs->name = "Enterance to " + level.levels[*j].name;
				 stairs->x = x;
				 stairs->y = y;
	 ...
	 // create stairs up
	 elevator = definitions.find_terrain("Stairs (up)");
...
void CMap :: CreateMapCaves()
{
	int x,y;
	int fill;

   CELL *predefined_floor=definitions.find_terrain("Floor");
   CELL *predefined_wall=definitions.find_terrain("Stone Wall");
...
bool CMap::onMap(int x, int y)
{
   return (x>=0 && x<MAPWIDTH && y>=0 && y<MAPHEIGHT);
}

int CMap::getCornerState(const int &x, const int &y)
{
	if (cornerOnMap(x,y))
		return corners[x][y];
	return 0;
...
void CMap::setCornerState(const int &x, const int &y, const int &state)
{
	if (cornerOnMap(x,y))
		corners[x][y]=state;
}

bool CMap::cornerOnMap(const int &x, const int &y)
{
	if (x>=0 && x<MAPWIDTH+1 && y>=0 && y<MAPHEIGHT+1)
		return true;
	return false;
...
bool CMap::blockLOS(int x, int y)
{
...
bool CMap::blockMove(int x, int y)
{
   if(!onMap(x,y)) return true;
...
bool CMap::blockMissile(int x, int y)
{
	if (isShield(x,y))
		return true;

	return blockMove(x,y);
}

void CMap::setBlockLOS(int x, int y)
{
   if(!onMap(x,y)) return;
   cells[x][y].block_los=true;
}

void CMap::backBlockLOS(int x, int y)
{
   if(!onMap(x,y)) return;
...
void CMap::setBlockMove(int x, int y)
{
   if(!onMap(x,y)) return;
   this->cells[x][y].block_move=true;
}

void CMap::backBlockMove(int x, int y)
{
   if(!onMap(x,y)) return;
...
void CMap::setSeen(int x, int y, bool state)
{
   if(!onMap(x,y)) return;
...
void CMap::setSeenByPlayer(int x, int y)
{
	if(!onMap(x,y)) return;
...
void CMap::setSeenByCamera(int x, int y)
{
	if(!onMap(x,y)) return;
...
bool CMap::isShield(int x, int y)
{
...
bool CMap :: isOpenable(int x, int y)
{
   if(!onMap(x,y)) return false;
...
bool CMap :: isClosable(int x, int y)
{
   if(!onMap(x,y)) return false;
...
void CMap::addShield(int x, int y)
{
   if(!onMap(x,y)) 
	   return;
...
int CMap::getShield(int x, int y)
{
...
void CMap::removeShield(int x, int y)
{
   if(!onMap(x,y)) return;

   assert(cells[x][y].shield>0);
   cells[x][y].shield--;
}

void CMap::MarkMonsterOnMap(int x, int y)
{
	if(!onMap(x,y)) return;	
...
void CMap::UnMarkMonsterOnMap(int x, int y)
{
	if(!onMap(x,y)) return;	
...
bool CMap::IsMonsterOnMap(int x, int y)
{
	if(!onMap(x,y)) return false;	
...
void CMap::IncraseNumberOfItems(int x, int y)
{
	if(!onMap(x,y)) return;	
	this->cells[x][y].number_of_items++;
}

void CMap::DecraseNumberOfItems(int x, int y)
{
	if(!onMap(x,y)) return;	
	this->cells[x][y].number_of_items--;
...
int CMap::GetNumberOfItems(int x, int y)
{
	if(!onMap(x,y)) return false;	
	assert(this->cells[x][y].number_of_items>=0);
...
void CMap::IncraseNumberOfGases(int x, int y)
{
	if(!onMap(x,y)) return;	
	this->cells[x][y].number_of_gases++;
}

void CMap::DecraseNumberOfGases(int x, int y)
{
	if(!onMap(x,y)) return;	
	this->cells[x][y].number_of_gases--;
...
int CMap::GetNumberOfGases(int x, int y)
{
	if(!onMap(x,y)) return false;	
	assert(this->cells[x][y].number_of_gases>=0);
...
bool CMap::AreStairs(int x, int y)
{
	if(!onMap(x,y)) return false;	
	return this->cells[x][y].is_stairs;
}

void CMap::MarkStairs(int x, int y)
{
	if(!onMap(x,y)) return;	
	this->cells[x][y].is_stairs=true;
}

void CMap::UnMarkStairs(int x, int y)
{
	if(!onMap(x,y)) return;	
...
bool CMap::destroyCell(int x, int y)
{
  CELL *found;
  if (!onMap(x,y))
    return false;
...
int CMap::getPercentDamage(int x, int y)
{
   int value;
   if(!onMap(x,y)) return false;
...
bool CMap::damage(int x, int y, int value)
{
   if(!onMap(x,y)) return false;
...
bool CMap::seen_once(int x, int y)
{
	if (x<0 || x>=MAPWIDTH || y<0 || y>=MAPHEIGHT)
		return false;
...
bool CMap::seen_by_player(int x, int y)
{
	if (x<0 || x>=MAPWIDTH || y<0 || y>=MAPHEIGHT)
		return false;
...
bool CMap::seen_by_camera(int x, int y)
{
	if (x<0 || x>=MAPWIDTH || y<0 || y>=MAPHEIGHT)
		return false;
...
void CMap::setLastTile(int x, int y, int tile)
{
   if(!onMap(x,y)) return;
...
string CMap :: getCellName(int x, int y)
{
   if (!onMap(x,y))
     return string("");
   ...
   for(x=0; x<MAPWIDTH; x++)
   {
      for(y=0; y<MAPHEIGHT; y++)
      {
         if(seen_by_player(x,y))
         {
            ...
            if (getShield(x,y) && !blockMove(x,y))
            {
...
bool CMap::AddDoorsInShuttle()
{
     ptr_list :: iterator m;
     ptr_list pozycje;

     CELL *wall=definitions.find_terrain("SpaceShip Wall");
	 CELL *door;
	 door=definitions.find_terrain("Drawing Door (closed)");

     int data[MAPWIDTH+1][MAPHEIGHT+1];
     struct CPosition *pos;
	 int x,y,xm,ym;
...
void CMap::CreateMapShuttle()
{
	...
	undef=definitions.find_terrain("SpaceShip Wall");
	floor=definitions.find_terrain("Floor");
	wall =definitions.find_terrain("SpaceShip Wall");
	door=definitions.find_terrain("Drawing Door (closed)");
	wares=definitions.find_terrain("Wares");
	boxes=definitions.find_terrain("Boxes");
	...
	for (number_of_rooms=0;number_of_rooms<max_number_of_rooms;)
	{
		...
		else
		{
			...
			// check, czy nie jest w istniej¹cym
	 ...
	 for (j = level.stairs_down[level.ID].begin(),_j = level.stairs_down[level.ID].end();j!=_j;j++)
	 {
		 ...
		 while(1)
		 {
			 ...
			 if (cells[x][y]==*floor && (cells[x-1][y]==*wall || cells[x+1][y]==*wall || cells[x][y-1]==*wall || cells[x][y+1]==*wall))
			 {
				 cells[x][y]=*elevator;
				 stairs->to_where = *j;
				 stairs->name = "Lift to " + level.levels[*j].name;
				 stairs->x = x;
				 stairs->y = y;
				 level.map.MarkStairs(stairs->x,stairs->y);					 
				 stairs->lead_up = false;
...
#define MC_CROSSROAD 1
#define MC_LIFETIME 200
#define MAX_EMPTY_COUNT MAPWIDTH*MAPHEIGHT/3

void CMap::CreateMapUnderground(bool interpolation)
{
   int x,y;
...
void CMap :: CreateMapCapsule()
{
	...
	CELL *wall=definitions.find_terrain("SpaceShip Wall");
	CELL *floor = definitions.find_terrain("Floor");	
	CELL *space = definitions.find_terrain("Space");
	CELL *far_space = definitions.find_terrain("Far Space");
	CELL *door=definitions.find_terrain("Drawing Door (closed)");
	CELL *door_bad = definitions.find_terrain("Closed Capsule Door");
	CELL *lift_up=definitions.find_terrain("Stairs (up)");
	CELL *lift_down=definitions.find_terrain("Stairs (down)");
	CELL *capsule=definitions.find_terrain("Rescue Capsule");
	CELL *type;
	STAIRS *stairs;
	...
	for (y=0;y<=14;y++)
	{
		for (x=0;x<mx[0].size();x++)
		{
			...
			case '<':
				...
				stairs_of_map.push_back(stairs);
				stairs->to_where = "s04";
				stairs->name = "Lift to Space Ship lev. 4";
				stairs->x = px;
				stairs->y = py;
			    level.map.MarkStairs(stairs->x,stairs->y);					 
				stairs->lead_up = true;
	...
	level.create_monster(ax,ay,"Genotroid",0,0);				
...
void CMap :: CreateMapCityNW()
{
	int x,y;
	int px,py;
	
	CELL *door  = definitions.find_terrain("Drawing Door (closed)");
	CELL *platform = definitions.find_terrain("Landing Platform");	
	CELL *capsule = definitions.find_terrain("Rescue Capsule");

	CELL *wall=definitions.find_terrain("Building Wall");
	CELL *dome = definitions.find_terrain("Dome");	
	CELL *door1 = definitions.find_terrain("Building Door (opened)");
	CELL *door2 = definitions.find_terrain("Building Door (closed)");
	CELL *floor = definitions.find_terrain("Floor");
	CELL *space = definitions.find_terrain("Space");
	CELL *enterance = definitions.find_terrain("Stairs (up)");
	...
	CELL *c_exit = definitions.find_terrain("Stairs (down)");
	STAIRS *stairs = new STAIRS;
	stairs_of_map.push_back(stairs);
	cells[MAPWIDTH/2][MAPHEIGHT-2]=*c_exit;
	stairs->to_where = "csw";
	stairs->name = "Gateway to Sector SW";
	stairs->x = MAPWIDTH/2;
	...
	c_exit = definitions.find_terrain("Stairs (up)");
	stairs = new STAIRS;
	stairs_of_map.push_back(stairs);
	cells[MAPWIDTH/2][1]=*c_exit;
	stairs->to_where = "gl0";
	stairs->name = "Enterance to Genetic Laboratory";
	stairs->x = MAPWIDTH/2;
	stairs->y = 1;
	level.map.MarkStairs(stairs->x,stairs->y);					 	
	stairs->lead_up = true;
...
void CMap :: CreateMapCitySW()
{
	...
	CELL *door2 = definitions.find_terrain("Building Door (closed)");
	CELL *floor = definitions.find_terrain("Floor");
	CELL *enterance = definitions.find_terrain("Stairs (down)");
	STAIRS *stairs;
	...
	CELL *c_exit = definitions.find_terrain("Stairs (up)");
	stairs = new STAIRS;
	stairs_of_map.push_back(stairs);
	cells[MAPWIDTH/2][1]=*c_exit;
	stairs->to_where = "cnw";
	stairs->name = "Gateway to Sector NW";
	stairs->x = MAPWIDTH/2;
	stairs->y = 1;
	level.map.MarkStairs(stairs->x,stairs->y);					 
	stairs->lead_up = true;
...
bool CMap :: FindRectangleOfType(CELL *type, int &px, int &py, int size_x, int size_y, int grid)
{
	int x,y;
	bool not_blocked;
...
void CMap::CollapseCeiling(int x,int y)
{
	CELL *zwalony=definitions.find_terrain("Collapsed Ceiling");
	for (int a=-1;a<=1;a++)
		for (int b=-1;b<=1;b++)
		{
			if (onMap(x+a,y+b))
...
void CMap::CreateMapMines()
{
	...
	CELL *wall=definitions.find_terrain("Stone Wall");
	CELL *floor = definitions.find_terrain("Floor");
	CELL *corridor = definitions.find_terrain("Space");
	CELL *cantilever = definitions.find_terrain("Cantilever");
	int random_value;
	int diff_x, diff_y;
	int px1,py1,px2,py2;
...
bool CMap::addDanger(int x, int y, int danger)
{
	if(onMap(x,y))
	{
...
bool CMap::CreateMapGeneticLabUnderground()
{
	...
	CELL *floor=definitions.find_terrain("Floor");
	CELL *corridor=definitions.find_terrain("Corridor");
	CELL *wall=definitions.find_terrain("Plastic Wall");
	CELL *door[5];
	door[0]=definitions.find_terrain("Plastic Door (opened)");
	door[1]=definitions.find_terrain("Plastic Door (opened)");
	door[2]=definitions.find_terrain("Metal Door (opened)");
	door[3]=definitions.find_terrain("Metal Door (closed)");
	door[4]=definitions.find_terrain("Corridor");
...
// stworzenie stairs
	...
	{
		elevator = definitions.find_terrain("Stairs (down)");
		STAIRS *stairs = new STAIRS;
		stairs_of_map.push_back(stairs);
		...
		while(1)
		{
			...
			if (cells[x][y]==*floor)
			{
				cells[x][y] = *definitions.find_terrain("Stairs (up)");
				cells[x][y].color = 14;
				STAIRS *stairs = new STAIRS;
				stairs_of_map.push_back(stairs);
				stairs->to_where = "Genetic Machine";
				stairs->name = "Genetic Machine";
				stairs->x = x;
				stairs->y = y;
				level.map.MarkStairs(stairs->x,stairs->y);					 
				stairs->lead_up = true;
...
void CMap::CreateMapBunker()
{
	int x,y;

	CELL *mwall=definitions.find_terrain("Metal Wall");
	CELL *pwall=definitions.find_terrain("Strange Metal Wall");
	CELL *floor = definitions.find_terrain("White Floor");
	CELL *swall = definitions.find_terrain("Stone Wall");
	CELL *type;

	CELL *door=definitions.find_terrain("Bunker Door");
	CELL *stairs_up=definitions.find_terrain("Stairs (up)");
	CELL *stairs_down=definitions.find_terrain("Stairs (down)");
	...
	for(x=0; x<18; x++)
		...
		for (y=0;y<8;y++)
		{
			for (x=0;x<mx[0].size();x++)
			{
				...
				case 'o':
					type = mwall;
					level.create_monster(x,y,"Searchlight",0,0);
					break;
				case '+':
					type = mwall;
					level.create_monster(x,y,"Bunker Gun",0,0);
					break;
				}
				//			if (type!=floor)
				cells[x][y]=*type;				
...
void CMap::CreateMapDoctorsCave()
{
	int x,y;
	...
	level.create_monster(x_of_doctor,y_of_doctor,"Mad Doctor",0,0);
	
	// stworzenie stairs

	///////////////////////////////////// TO TESTOWE
	// create stairs up
	CELL *elevator = definitions.find_terrain("Stairs (up)");
	// tylko jedne schody na dole map
	list <string>::iterator j,_j;
	j = level.stairs_up[level.ID].begin();
	STAIRS *stairs = new STAIRS;
...
void CMap :: CreateMapTutorial()
{
	...
	// 1. Wstêp, poruszanie, help, message buffer
	// 1.1 Otwieranie drzwi
	// 2. Podnoszenie przedmiotów, inwentarz
	// 3. Otwieranie drzwi
	// 4. Look, walka
	// 5. Broñ, amunicja
	// 6. Tryby strzelania
	...
	CELL *wall=definitions.find_terrain("Metal Wall");
	CELL *floor = definitions.find_terrain("Floor");	
	CELL *door=definitions.find_terrain("Plastic Door (closed)");
	CELL *door2=definitions.find_terrain("Metal Door (closed)");
	CELL *boxes = definitions.find_terrain("Boxes");	
	CELL *lift_down=definitions.find_terrain("Stairs (down)");
	CELL *type;
	...
	for (y=0;y<30;y++)
	{
		for (x=0;x<mx[0].size();x++)
		{
			...
			case '$':
				...
				break;
			case 'g':
				level.create_item(px,py,"Hand Grenade",0,0);
				level.create_item(px,py,"Hand Grenade",0,0);
				level.create_item(px,py,"Smoke Grenade",0,0);
				type = floor;
				break;
			case 'p':
				level.create_item(px,py,"Nanorebuilding Pill",0,0);
				level.create_item(px,py,"Nanorebuilding Pill",0,0);
				type = floor;
				break;
			case 'E':
				level.create_item(px,py,"Power Armor",0,0);
				level.create_item(px,py,"Self-Charging Accumulator",0,0);
				level.create_item(px,py,"Chainsaw",0,0);
				type = floor;
				break;
			case 'U':
				cells[px][py]=*type;
				level.create_monster(px,py,"Utility Bot",0,0);
				type = floor;
				break;
			case 'C':
				cpu = (CProcessor *) level.create_item(px,py,"Weak Simple Seeker CPU",0,0);
				if (cpu!=NULL)
					cpu->group_affiliation = GROUP_HERO;
				type = floor;
				level.create_item(px,py,"Rubber Robot Wheel",0,0);
				break;
			case 'M':
				type = floor;
				cells[px][py]=*type;
				level.create_monster(px,py,"Tutorial Monster",0,0);
				break;
			case 'O':
				type = floor;
				cells[px][py]=*type;
				level.create_monster(px,py,"Tutorial Target",0,0);
				break;
			case '@':
				level.player->ChangePosition(px,py);
			break;
			case ']':
				level.create_item(px,py,"Trooper Armor",0,0);
				type = floor;
				break;
			case '/':
				level.create_item(px,py,"LR-30 Assault Rifle",0,0);
				type = floor;
				break;
			case '|':
				level.create_item(px,py,"Rifle Bullet 6.25mm",50,50);
				type = floor;
				break;
			case '>':
				type = lift_down;
