...
{ 
	...
	if (owner!=NULL && type == ATTRIB_TYPE_SPD)
	{
		...
		if (intel!=NULL)
		{
			// linear lowering the speed when burdened
			if (intel->isBurdened())
			{
				int min = intel->get_monster_capacity()*3/2;
				min -= intel->get_backpack_weight();
				int max = intel->get_monster_capacity();
				value=(min*value)/max;
			}
		}
	}
...
CEnemy *CMonster::IsHostile()
{
	return dynamic_cast<CEnemy *>(this);
}

CIntelligent *CMonster::IsIntelligent()
{
	return dynamic_cast<CIntelligent *>(this);
}

CAnimal * CMonster::IsAnimal()
{
	return dynamic_cast<CAnimal *>(this);
};

CMadDoctor * CMonster::IsMadDoctor()
{
	return dynamic_cast<CMadDoctor *>(this);
};

CRobot * CMonster::IsRobot()
{
	return dynamic_cast<CRobot *>(this);
};

CHero * CMonster::IsPlayer()
{
	return dynamic_cast<CHero *>(this);
};

CSearchLight * CMonster::IsSearchlight()
{
	return dynamic_cast<CSearchLight *>(this);
}

CWorm * CMonster::IsWorm()
{
	return dynamic_cast<CWorm *>(this);
};


bool CMonster::is_enemy(CMonster *m)
{
...
bool CMonster::ChangePosition(int x, int y)
{
...
bool CMonster::is_friendly(CMonster *m)
{
	return (group_affiliation == m->group_affiliation);
}

int CMonster::mutate(int type)
{
	if (this->seen_now)
	{
		level.player->stop_repeating();				  							
		screen.console.add(name + " mutates!",14);
	}
...
bool CMonster::heal(int damage)
{
  ...
  if (hit_points.val >= hit_points.max)
  {
	...
	{
		screen.console.add("Fully rested.",7); // this stops repeating
	}
  }
  return healed;
}

void CMonster::self_treatment_every_turn()
{
...
void CMonster::status_management_every_turn()
{
	int endurance_val;
	int value;
	...
	if (state[STATE_CHEM_POISON]>0)
	{
		...
		else if (lower_random(state[STATE_CHEM_POISON]*2,150))
		{
			...
			if (value>0)
			{
				...
				{
					level.player->stop_repeating();				  					
					screen.console.add(name + " suffers from poison.",2);
					if (cause_damage(value)==DAMAGE_DEATH)
						screen.console.add(name + " died poisoned.",4);
				}
			}
		}
	}
	...
	if (state[STATE_TEMPERATURE]>0)
	{
		...
		if (lower_random(state[STATE_TEMPERATURE]*3,100))
		{
			...
			{
				level.player->stop_repeating();				  					
				screen.console.add(name + " is burning.",12);
				if (cause_damage(value)==DAMAGE_DEATH)
					screen.console.add(name + " died fried.",4);
			}
		}
	}
	...
	if (state[STATE_TEMPERATURE]<0)
	{
		...
		{
			level.player->stop_repeating();				  					
			screen.console.add(name + " suffers from cold.",15);
			if (cause_damage(value)==DAMAGE_DEATH)
				screen.console.add(name + " died frozen.",4);
		}
	}
...
void CMonster::wait (t_time t)
{
  next_action_time=level.turn+t;
}

void CMonster::cell_in_current_direction(int &x, int &y)
{
  switch (direction)
  {
      case _kn:
...
void CMonster::direction_to_near_cell(int x, int y) 
{
      if (x>pX() && y==pY())
        direction=_ke;
      else if (x<pX() && y==pY())
...
int CMonster::cause_damage(int damage)
{
   string text;
   ...
   else
   {
	   if (this->seen_now)
	   {
		   CMonster *sound=(CMonster *) definitions.find_monster(this->name);
		   if (sound!=NULL)
...
int CMonster::get_h2h_attack_value()
{
	int attack;
	if (weapon==NULL)
		return 0;
...
int CMonster::get_h2h_defense_value()
{
	int defense;
	if (weapon==NULL)
		return 0;	
...
void CMonster::attack(CMonster *a_enemy)
{
   int attack, defense;
   CItem *weapon;
   CItem *enemy_weapon;
   ...
   if (lower_random(defense,defense+attack*3))
   {
     if (this->seen_now==true) // evaded
     {
		 if (a_enemy->seen_now)
		 {
			text=name + " missed " + a_enemy->name + ".";
			sounds.PlaySoundOnce("data/sounds/other/miss.wav");		
		 }
		 else
			text=name + " missed something.";
        screen.console.add(text,7);
     }
   }
   else // hit
   {
	 ...
	 {
		 if (with_damage!=-1)
			descriptions.add_attack("HIT"," for " + IntToStr(with_damage) + " damage");
		 else
	 ...
	 if (this->seen_now==true)
	 {
		 ...
		 text=name + " " + t + " " + something + descriptions.get_attack("HIT") + ".";
		 if (text.find("killing")!=-1)
				...
				{					
					if ((monster==level.player || this==level.player) || (coin_toss() && coin_toss())) // gdy player, to zawsze wymiana, gdy other, to 25% szans na to.
					{
						...
						if (this->seen_now || monster->seen_now)
							screen.console.add(name + " and " + monster->name + " swap places.",7);
					}
					else
						return false;
				}
...
bool CMonster::attack_in_direction()
{
	CMonster *monster;
	int x,y;
	cell_in_current_direction(x,y);
	...
	if (this->seen_now)
	{
		CItem *sound=(CItem *) definitions.find_item(weapon->name);
		if (sound!=NULL)
			sounds.PlaySoundOnce(sound->sound_used);

		if (power>0)
			screen.console.add(name + " attacks " + level.map.getCellName(x,y) + ".",7);	
		else
			screen.console.add(name + " attacks " + level.map.getCellName(x,y) + " without any result.",7);	
	}
...
CMadDoctor::CMadDoctor()
{
	...
	histext[0] = "You won't get me ugly bastards!";
	histext[1] = "Mwahahahaha!";
	histext[2] = "Try this!";
	histext[3] = "Are you happy now???";
	histext[4] = "Die!";
	histext[5] = "Oh, I feel dead now... Haha! You can't kill me!";
	histext[6] = "Damn aliens! You won't take me alive!";
	histext[7] = "Do you like my new form? Haha!";
	histext[8] = "You will die!";
	histext[9] = "I'm gonna kill ya!";
	histext[10] = "I'm gonna kill ya, then I'm gonna cook ya, then I'm gonna eat you!";
	histext[11] = "You're already dead!";
	histext[12] = "Die you bewinged spawn of Satan!";
	histext[13] = "Nobody kills anybody in my place of business except me!";
	histext[14] = "Don't you people ever die?";
	histext[15] = "What now? Save the girl? You'll have to kill me, Charles! Haha!";
	histext[16] = "The defense department regrets to inform you that your sons are dead because they were stupid!";
	histext[17] = "Yeah, I'm going to kill you pretty soon!";
	histext[18] = "You're already dead, Silberman! Already dead!";
	histext[19] = "They're coming outta the goddamn walls!";
	histext[20] = "You'll be dead! Haha! Hahaha!";
	histext[21] = "He will join us or die, my master!";
	histext[22] = "I will not turn, and you'll be forced to kill me!";
	histext[23] = "The Mondoshawans never fully trusted the human race!";
	histext[24] = "I dreamed I killed you and Danny. But I didn't just kill ya. I cut you up in little pieces! Hahaha!";	

	yelling_chance = 0;
}

void CEnemy::monster_sees_enemy(CMonster *enemy)
{
		if (enemy!=NULL)
		{
...
int CEnemy::set_direction_to_cell_by_shortest_path(int cell_x,int cell_y,bool open_doors)
{
     int cells,a;
     ...
     bool way_blocked;
     struct CPosition position[100]; // so much for the map
     travel_distance=cells=generate_bresenham_line(this->pX(),this->pY(),cell_x,cell_y, (CPosition *) position, 100);
...
bool CEnemy::go_around_cell(int x,int y, bool opening) // returns if opened door
{
         int random_value;
         CMonster *monster;
...
EAction CAnimal::get_action() // AI of animal
{
	  if (IsStunned())
...
EAction CWorm::get_action() // AI of giant worm
{
...
int CAnimal::cause_damage(int damage)
{
	...
	return CMonster::cause_damage(damage);
}

bool CAnimal::look_around()
{
	...
	CMonster::look_around(); // to handle cameras and searchlights
	...
	if (!sees_enemy_again)
	{
		...
		{
			actual_behaviour=BEHAVIOUR_H2H_COMBAT;
			enemy = (CMonster *) *seen_enemies.begin();
...
t_time CAnimal::do_action( EAction action )
{
   last_pX = pX();
   last_pY = pY();
...
t_time CWorm::do_action( EAction action )
{
	last_pX = pX();
	last_pY = pY();
...
bool CSearchLight::death()
{	
	return CMonster::death();
}


EAction CSearchLight::get_action() // AI reflektora
{
	level.map.UnSeenMap();
	look_around();
    return ACTION_WAIT;
...
t_time CSearchLight::do_action( EAction )
{
	if (enemy==NULL)
	{
		this->color=14;
		...
		else // ruch w nowe place
		{
			...
			int cells;
			cells=generate_bresenham_line(last_x_of_enemy,last_y_of_enemy, destination_x, destination_y, (CPosition *) position, 100);
...
bool CSearchLight::look_around()
{
	...
	for (int a=last_x_of_enemy-5;a<last_x_of_enemy+5;a++)
	{
		...
		cells=generate_bresenham_line(this->pX(),this->pY(),targetx,targety, (CPosition *) position, 100);
		for (index=1;index<cells;index++)
		{
			...
			else
			{
				if (position[index].y>last_y_of_enemy-4 && position[index].y<last_y_of_enemy+4)
				{
					...
					if (enemy==NULL)
					{
						CMonster *monster1=level.monster_on_cell(position[index].x,position[index].y);
						if (monster1!=NULL)
						{
...
bool CIntelligent::throw_item_to (int x, int y, CItem *item)
{
  int accuracy;
  ...
  int result;
     ...
     if (this->seen_now==true)
     {
        screen.console.add(name + " throws " + item->show_name() + ".",3);
     }
  ...
  if (max_range>80)
   max_range=80;
     ...
     struct CPosition position[100]; // struktura opisujaca wyliczony tor lotu
     int cells, index;
     CMonster *monster;
     bool zablokowany;

     // count liczby cells to target
     cells=generate_bresenham_line(start_x,start_y, x, y, (CPosition *) position,100);
     // count straight do rzeczywistego celu
     generate_bresenham_line(start_x, start_y, target_x, target_y, (CPosition *) position,100);
          ...
          monster=level.monster_on_cell(position[index].x,position[index].y);
			 ...
			 if (monster!=NULL && monster->evade_missile(this)==false) // nie uniknal
			 {
				   ...
				   if (result>0 && monster->seen_now)
					   screen.console.add(item->show_name() + " hits " + monster->name + " for " + IntToStr(result) + " damage.",6);
...
//				 myrefresh();
				 ...
				 if (level.map.seen_by_player(start_x,start_y))
					 sounds.PlaySoundOnce("data/sounds/other/bounce.wav");						 
			 }
        }
...
int CIntelligent::shoot_into (int x, int y)
{
  int accuracy;
  int target_x, target_y;
  int random_value;
  ...
  if (this->seen_now) // on bunker enterance we hear everything
	  sound =(CRangedWeapon *) definitions.find_item(range_weapon->name);
  else
	  sound=NULL;
  switch(range_weapon->fire_type)
  {
...
int CIntelligent::get_backpack_weight()
{
   ptr_list::iterator m,_m;
   CItem *item;
   int weight;
...
bool CIntelligent::choose_ready_weapon(CItem *item)
{
	if (item==NULL)
		return false;
...
bool CIntelligent::set_ready_weapon()
{
	CItem *item;
...
int CIntelligent::reload_weapon()
{
   ptr_list::iterator m,_m;
   CItem *item;
   bool found;
      ...
      if ( !weapon->IsLoaded() ) 
      {
		...
		if (found==true)
		{
			...
			if (ranged_weapon->load_ammo(am)==0)
			{
				if (this->seen_now==true)
				{
					screen.console.add(this->name + " has reloaded weapon" + other,3);
...
int CIntelligent::pick_up_item(CItem *item, bool see_possible)
{
   string text;
 ...
 //   1. Find the same in backpack
	  ...
	  if (found) // the same
	  {
          ...
          {
			  screen.console.add(this->name + " has picked up " + item->article_the() + item->show_name() +".",3);
...
int CIntelligent::use_item(CItem *item)
{
	return item->use_on(this);
}
	
int CIntelligent::take_out_from_backpack(CItem *item)
{
   if (item==NULL)
     return false;
...
int CIntelligent::drop_item(CItem *item, bool see_possible)
{
   string text;
   if (item==NULL)
     return false;
   ...
   if (see_possible)
     if (this->seen_now==true)
     {
       text=this->name + " has dropped " + item->article_the() + item->show_name() + ".";
...
int CIntelligent::get_monster_capacity()
{
   return strength.GetValue()*5000;
}

int CIntelligent::can_pick_up(CItem *item)
{
...
int CIntelligent::set_weapon(CItem *item)
{
   string text;
   if (item==NULL)
     return false;
   ...
   if (this->seen_now==true)
   {
     text=this->name + " is using " + item->show_name() + " as weapon now.";
     screen.console.add(text,3,false);
   }
   return true;
}

int CHero::set_weapon(CItem *item)
{
	if (item==NULL)
		return false;

	if (unarmed.name=="Bare Hands")
		return CIntelligent::set_weapon(item);

	screen.console.add("You have no hands to wield!",3);
	return false;
}

int CIntelligent::remove_armor()
{
	if (!this->in_armor())
		return false;
...
int CIntelligent::set_armor(CItem *item)
{
   string text;
   if (item==NULL)
     return false;
   ...
   {
	   screen.console.add(this->name + " can't put on the " + item->show_name(),3);	   
	   return false;
   }
   

   this->armor = (CArmor *) item;
      
   if (this->seen_now==true)
   {
     if (this->in_armor())
       text=this->name + " puts on " + item->show_name() + ".";
     else
       text=this->name + " is without armor now.";
...
bool CMonster::death()
{
 ...
 // pozostawienie ciala
   if (is_enemy(level.player))
   {
	 ...
	 if (special_properties&SPECIAL_PROPERTY_SPLITTER && hit_points.max>5)
	 {
		 ...
		 for (int a=0;a<10;a++)
		 {
			 ...
			 if (level.entering_on_cell_possible(posx,posy))
			 {
				 ...
				 if (this->seen_now)
				 {
					 screen.console.add(this->name + " splits!",14);
				 }

				 CMonster *new_one = this->duplicate();
	 ...
	 {
		 CCorpse *corpse;
		 corpse=(CCorpse *) level.create_base_item("CORPSE");
		 
		 corpse->color=this->color;
		 corpse->of_what=this->name;
		 corpse->rename(corpse->of_what + " Corpse");
...
void CMonster::enemy_died()
{
	enemy=NULL;
}

void CEnemy::enemy_died()
{
	last_x_of_enemy = pX();
	last_y_of_enemy = pY();
	CMonster::enemy_died();
...
bool CMonster::evade_missile(CMonster *thrower)
{
  int hit;
  int evade;
  ...
  if (thrower==NULL) 
  {
    ...
    {
      if (this->seen_now==true)
        screen.console.add(name +" dodged.",7);
      return true;
    }
  }
  else
...
bool CIntelligent::evade_missile(CMonster *thrower)
{
   return CMonster::evade_missile(thrower);
}

bool CAnimal::evade_missile(CMonster *thrower)
{
   return CMonster::evade_missile(thrower);
}

int CMonster::hit_by_item(int hit_power, CItem *item)
{
	int normal_power;
	string descr;
    ...
    if (this->in_armor())
	{
		// hit this armor;
		  ...
		  if (item->IsAmmo() && item->properties&TYPE_ARMOR_PERCING) // when missile is AP
		  {
			 ...
			 {
				 CBaseArmor *parmor = armor->IsArmor();
...
int CMonster::hit_by_explosion(int hit_power)
{
	if (hit_power==0)
		return 0;
...
int CMonster::hit_changes_status(int kind, int power)
{
	if (power<=0)
		return 0;
	...
	else if (kind&TYPE_RADIOACTIVE)
	{
		...
		if (power>0)
		{
			...
			if (this->seen_now==true)
				screen.console.add(name + " has been radiated.",14);
		}
	}
	else if (kind&TYPE_CHEM_POISON)
	{
		...
		if (power>0)
		{
			...
			if (this->seen_now==true)
				screen.console.add(name + " has been poisoned.",2);
		}
	}
	else if (kind&TYPE_STUN)
	{
		...
		if (power>state[STATE_STUN])
		{
			...
			if (this->seen_now==true)
				screen.console.add(name + " has been stunned.",8);
		}
	}
	else if (kind&TYPE_ELECTRIC)
	{
		...
		if (power>0)
		{
			...
			if (this->seen_now==true)
				screen.console.add(name + " has been shock8ed for " + IntToStr(power) + " damage.",5);
		}
	}
	else if (kind&TYPE_PARALYZE)
	{
		...
		if (power>state[STATE_SPEED] && power>0)
		{
			...
			if (this->seen_now==true)
				screen.console.add(name + " has been slowed.",1);
		}
	}
	return 0;
}
...
bool CIntelligent::open_in_direction()
{
  int x,y;
  string old_one;
  cell_in_current_direction(x,y);
  ...
  if (level.map.open(x,y)==true && this->seen_now==true)
  {
	  if ((this==level.player && !level.player->is_repeating_action()) || this!=level.player)
		screen.console.add(name + " has opened the " + old_one +".",3);
    return true;
  }
  return false;
}

bool CIntelligent::close_in_direction()
{
  int x,y;
  string old_one;
  cell_in_current_direction(x,y);
  ...
  if (level.map.IsMonsterOnMap(x,y)==false)
  {
    ...
    {
      screen.console.add(name + " has closed the " + old_one +".",3);
      return true;
    }
  }
  return false;
...
CMonster * CIntelligent::duplicate()
{
   CIntelligent *new_one;
   CItem *temp, *temp2;
   ptr_list::iterator m, _m;
   
   new_one=new CIntelligent;
   CItem *old_armor= this->armor;
  ...
  for(m=this->backpack.begin(),_m=this->backpack.end(); m!=_m; m++)
  {
         ...
         {
		   ((CIntelligent *)new_one)->set_armor(temp2);
         }
  ...
  if (definitions.lista_kategorii_losowych_itemow_potwora.find(name)!=definitions.lista_kategorii_losowych_itemow_potwora.end())
  {
   ...
   return (CMonster *) new_one;
}

int CMonster::use_energy(int energy)
{
...
int CMonster::add_energy(int energy)
{
	if (energy_points.val == energy_points.max)
		return false;
	...
	if (level.is_player_on_level && this==level.player)
		if (energy_points.val == energy_points.max)
		{
			screen.console.add("Energy regenerated.",7); // this message will stop repeating
		}
...
void CMonster::set_attributes_on_self()
{
...
CMonster * CSearchLight::duplicate()
{
	CSearchLight *new_one;
...
CMonster * CRobot::duplicate()
{
	...
	*new_one=*this;
	...
	// duplicate shell
	assert (shell!=NULL);
	new_one->shell = (CRobotShell *) shell->duplicate();
...
int CIntelligent::cause_damage(int damage)
{
	if (actual_behaviour == BEHAVIOUR_CALM)
		actual_behaviour = BEHAVIOUR_SEARCH_FOR_ENEMY;
	...
	for(m=this->backpack.begin(),_m=this->backpack.end(); m!=_m; m++)
	{
		...
		if (gr!=NULL && gr->active)
		{
			if (lower_random(damage,hit_points.val))
			{
				...
				{
					level.player->stop_repeating();					
					screen.console.add(this->name + " couldn't hold " + temp->article_a() + temp->show_name() + " and drop it.",12);
				}
				break; // drop only one
			}
		}
...
EAction CIntelligent::get_action() // AI of intelligent
{
	if (IsStunned())
	{
#ifdef DEBUG_AI
...
//	3. Rzuæ we of enemy active granat z backpacka. Je¿eli of enemy nie ma,
//		to rzuæ go gdzieœ daleko.
...
//	2. Je¿eli dalej ma³o HPków to przejdŸ do UCIECZKI
...
//	II. FAZA WSPOLNA DLA WALKI
		...
		actual_behaviour==BEHAVIOUR_RANGED_COMBAT || turned_to_fight)
	{
		...
		//	1. U¿yj boosterów, gdy masz jakieœ.
		...
		//	2. Zmieñ broñ na lepsz¹ (tego samego typu), gdy masz (wyceñ item).
		...
		if (!(AI_restrictions & AI_DONT_USE_GRENADES))
		{
			...
			if (random(random_value)==0)
			{
				...
				{
					if (this->seen_now==true)
						screen.console.add(name + " activates something!",12);												
					return ACTION_INVENTORY;
				}
			}
		} // gdy nie zabronione					
...
//	III. WALKA NA ODLEGLOSC
		...
		actual_behaviour==BEHAVIOUR_TURN_TO_FIGHT)
	{
		...
		//	2. Przeladuj weapon, gdy number pociskow = 0. Je¿eli nie masz wiecej pociskow,
		//	zmieñ broñ na inn¹ strzelaj¹c¹ (która jest zaladowana!).
		//	Je¿eli nie masz takiej broni przejdŸ do WALKI W ZWARCIU.
...
//	V. SPOKÓJ
//	1. Zaladuj niezaladowan¹ broñ strzelaj¹c¹. Je¿eli pocisków do niej w backpacku
//	jest wiêcej ni¿ pojemnoœæ magazynka i gdy magazynek nie pe³ny, to roz³aduj
//	broñ.
...
bool CIntelligent ::throw_item_to()
{
		int x,y;
		...
		if (!change_direction)
		{
			...
			// szukamy nowego, gdzie mo¿e byæ taki sam, jak przeciwny do old_direction
			for (a=0;a<8;a++)
			{
				direction = directions[a];
				cell_in_current_direction(x,y);
...
t_time CIntelligent::do_action( EAction action )
{
	last_pX = pX();
	last_pY = pY();
...
int CIntelligent::go_to_direction_of_enemy() // zwraca, czy powinien otworzyc drzwi
{
	if (AI_restrictions & AI_DONT_MOVE)
		return false;
...
bool CIntelligent::look_around()
{
	...
	level.fov.Start(&level.map,pX(),pY(),fov_radius.GetValue());
	CMonster::look_around(); // obsluga kamer i reflektorow	
	...
	if (weapon->property_load_ammo() && ranged_weapon!=NULL)
	{
		if ( !weapon->IsLoaded() ) // gdy weapon nie jest zaladowana
		{
			...
			if (found==true)
			{
				...
				{
					if (this->seen_now==true)
						screen.console.add(name + " has loaded weapon.",3,false);
				}
				return true;
			}              
		} // endof if lp==0
...
bool CIntelligent::unload_weapon()
{
	if (AI_restrictions & AI_DONT_USE_RANGED_WEAPONS)
		return false;
...
bool CIntelligent::fire_at_enemy()
{
	if (AI_restrictions & AI_DONT_USE_RANGED_WEAPONS)
		return false;
	...
	int cells, index;
	cells=generate_bresenham_line(pX(), pY(), x, y, (CPosition *) position,100);
	...
	if (!it_blocks)
	{
		...
		{
			level.player->stop_repeating();			
			screen.console.add(name + " fires!",3);								
		}
...
bool CIntelligent::use_healing_item()
{
	if (AI_restrictions & AI_DONT_USE_HEALING)
		return false;
	...
	for(m=backpack.begin(),_m=backpack.end(); m!=_m; m++)
	{
		...
		if (temp->property_use())
		{
			...
			{
				if (this->seen_now)
					screen.console.add(name + " ate something.",2);				
				p->use_on(this);
				return true;
			}
		}
...
bool CIntelligent::use_remove_poison_item()
{
	if (AI_restrictions & AI_DONT_USE_HEALING)
		return false;
...
bool CIntelligent::throw_active_grenade()
{
	if (AI_restrictions & AI_DONT_USE_GRENADES)
		return false;
...
bool CIntelligent::choose_place_to_throw_grenade(int &x,int &y)
{
	list < CPosition > position_list;
	list < CPosition >::iterator m,_m;
		...
		for (m=position_list.begin(),_m=position_list.end();m!=_m;m++)
		{
			...
			if (dist<range)
			{
				if (dist<lowest_distance)
				{
					...
					cells=generate_bresenham_line(pos.x, pos.y, last_x_of_enemy, last_y_of_enemy, (CPosition *) position, range);
					it_blocks = false;
...
bool CIntelligent::tell_others_about_enemy()
{
	ptr_list::iterator m,_m;
	CMonster *temp;
	...
	for (m=seen_friends.begin(),_m=seen_friends.end();m!=_m;m++)
	{
		...
		{
			CIntelligent *in = dynamic_cast <CIntelligent *> (temp);
			if (in!=NULL)
			{
...
bool CIntelligent::use_booster_item()
{
	if (AI_restrictions & AI_DONT_USE_BOOSTERS)
		return false;
...
bool CIntelligent::change_armor_to_better_one()
{	
...
bool CIntelligent::change_weapon_to_better_one()
{	
	if (AI_restrictions & AI_DONT_USE_OTHER_WEAPONS)
		return false;
...
int CIntelligent::activate_weapon_and_armor_when_energy_available()
{
	ptr_list::iterator m,_m;
	CItem *temp;
	CHandWeapon *hand_weapon;
	...
	{
		screen.console.add(name + " has activated his " + temp->show_name(),12);
		return true;
	}	
	return 0;
}

bool CIntelligent::activate_grenade_when_enemy_is_far()
{
	if (AI_restrictions & AI_DONT_USE_GRENADES)
		return false;
...
int CIntelligent::carried_number_of_missiles_for_weapon(CRangedWeapon *ranged_weapon)
{
	ptr_list::iterator m,_m;
	CItem *item;
	bool found;
...
bool CIntelligent::death()
{
	CItem *item;
	// drop items
	while (backpack.size()>0)
	{
		item=(CItem *) *backpack.begin();
		take_out_from_backpack(item);
	}// endof for
...
CItem * CIntelligent::go_to_valuable_item()
{
	if (AI_restrictions & AI_DONT_MOVE ||
...
int CIntelligent::pick_valuable_items()
{
	CItem *to_pick_up;
...
bool CIntelligent::find_best_weapons(CItem *&best_for_h2h, int &best_evaluation_h2h, CItem *&best_for_ranged, int &best_evaluation_ranged)
{
	ptr_list::iterator m,_m;
	CItem *item;
	CRangedWeapon *ranged_weapon;
...
bool CIntelligent::choose_run_away_direction()
{
	int x,y;
	int a,b,angle;
...
bool CIntelligent::use_defensive_techniques_running_away()
{
	// like mines, closing doors
	return false;
}

void CIntelligent::start_to_run_away()
{
	if (actual_behaviour!=BEHAVIOUR_RUN_AWAY)
	{
		run_away_to_x = pX();
...
int CIntelligent::how_danger_is_cell(int x,int y)
{
	ptr_list::iterator m,_m;
	GAS *gas;
	int how_danger=0;
...
int CAnimal::how_danger_is_cell(int x,int y)
{
	ptr_list::iterator m,_m;
	GAS *temp;
...
bool CIntelligent::defend_self_from_danger_on_map()
{
	int danger;
	ptr_list::iterator m,_m;
	CItem *item;
...
int CRobot::get_robot_speed()
{
	...
	size_t divider = (shell->move_slots.size());
...
t_time CRobot::do_action( EAction action )
{
	last_pX = pX();
	last_pY = pY();
	...
	if (sees_creator)
	{
		if (random(6)==0)
		{
			...
			{
				if (this->seen_now)
					sounds.PlaySoundOnce("data/sounds/other/robot_almost_destroyed.wav");
				screen.console.add(name + " reports: 'My " + shell->show_name() + " is almost destroyed!'",15);
			}
			else if (have_low_hp())
			{
				if (this->seen_now)
					sounds.PlaySoundOnce("data/sounds/other/robot_damaged.wav");
				screen.console.add(name + " reports: 'My " + shell->show_name() + " is heavily damaged!'",15);
			}	
		}
	}		
...
bool CRobot::look_around()
{
	...
	level.fov.Start(&level.map,pX(),pY(),fov_radius.GetValue());
	CMonster::look_around(); // handle cameras and searchlights

	// utility bot

	if (shell->cpu->name=="Utility Bot CPU")
	{
		for(m=level.items_on_map.begin(),_m=level.items_on_map.end(); m!=_m; m++)
		{
			item = (CItem *) *m;
...
bool CRobot::load_ammo_to_weapon()
{
	ptr_list::iterator m,_m;
...
char CRobot::get_CPU_instruction()
{
	char instruction;
	...
	size_t program_size = shell->cpu->program.size();
	...
	instruction=0;
	int index;
	for (size_t instruction_number=0;instruction_number<program_size;instruction_number++)
	{
		...
		case 'e': // go to nearest TRASH and utility it (treat trash an enemy) - for Utility Robot
			...
			{
				item = (CItem *) *m;
				if (item->name == "Garbage Cleaner")
					break;
			}
			// if doesn't have it, then skip instruction
			if (item==NULL || item->name!="Garbage Cleaner")
				continue;
				...
				if (items_on_cell.size()>0)
				{
					for (m=items_on_cell.begin(),_m=items_on_cell.end();m!=_m;m++)
					{
						...
						if (item->IsCorpse() || item->IsGarbage())
						{
							if (this->seen_now==true)
							{
								screen.console.add(this->name + " has utilized " + item->article_a() + item->show_name(),7);
								sounds.PlaySoundOnce("data/sounds/other/cleaner.wav");
							}
							item->death();
							return 'n';	// wait
						}
...
EAction CRobot::get_action() // AI of robot
{
...
bool CRobot::attack_in_direction()
{
	CMonster *monster;
	int x,y;
	cell_in_current_direction(x,y);
...
int CRobot::hit_changes_status(int kind, int power)
{
		...
		if (shell!=NULL && shell->cpu!=NULL)
		{
			if (power/4>0)
			{
				if (shell->cpu->damage_it(power/2)==DAMAGE_DEATH)
					...
					screen.console.add(name + " has been shocked.",5);			
			}
		}
	}
	return 0;
...
/// implement it better!!!
int CRobot::hit_by_explosion(int hit_power)
{
	return cause_damage(hit_power);
}

int CRobot::hit_by_item(int hit_power, CItem *item)
{
...
int CRobot::cause_damage(int damage)
{
	...
	if (damage<=0)
	{
		if (this->seen_now==true)
		{
			string text=name +" gains no wounds.";
			screen.console.add(text,7);
		}
		return DAMAGE_NONE;
	}
	...
	if (which_damage>0) // if shell destroyed, robot dies
	{
		...
		if (!item->IsCountable() && is_friendly(level.player) && this->sees_creator)
		{
			if (hp_before - which_damage < item->max_hit_points/4 && hp_before>=item->max_hit_points/4)
			{
				if (this->seen_now)
					sounds.PlaySoundOnce("data/sounds/other/robot_almost_destroyed.wav");
				descriptions.add_attack("ROBOTSAYS",name + " reports '" + "My " + item->show_name() + " is almost destroyed!'");
			}			
			else if (hp_before - which_damage < item->max_hit_points/2 && hp_before>=item->max_hit_points/2)
			{
				if (this->seen_now)
					sounds.PlaySoundOnce("data/sounds/other/robot_damaged.wav");
				descriptions.add_attack("ROBOTSAYS",name + " reports '" + "My " + item->show_name() + " is heavily damaged!'");
			}
		}
	}
	else if (which_damage==DAMAGE_DEATH)
	{
		if (item!=shell && shell!=NULL)
		{
			...
			if (this->seen_now)
				sounds.PlaySoundOnce("data/sounds/other/robot_almost_destroyed.wav");
			if (!item->IsCountable() && is_friendly(level.player) && this->sees_creator)
				descriptions.add_attack("ROBOTSAYS",name + " says: '" + "My " + item->show_name() + " is destroyed!'");
		}
		else
		{
...
void CRobot::attack(CMonster *enemy)
{
	ptr_list::iterator m,_m;
...
void CRobot::status_management_every_turn()
{
	return;
}

void CRobot::self_treatment_every_turn()
{
	return;
}

void CRobot::display()  // robot look like his shell
{ 
	tile=shell->tile; 
	color=shell->color; 
...
void CRobot::ShowDescription()
{
	...
	print_text(2,y++,string("Shell: ") + shell->show_name() + " <" + IntToStr(100*shell->hit_points/shell->max_hit_points) + "%>");
	y++;
		
	if (shell->action_slots.size()>0)
	{
		print_text(2,y++,string("Action Slots: ") + IntToStr(shell->action_slots.size()) + "/" +IntToStr(shell->max_number_action_slots) );
		y++;
		for (m=shell->action_slots.begin(),_m=shell->action_slots.end();m!=_m;m++)
		{
			temp2 = (CItem *) *m;
	...
	if (shell->move_slots.size()>0)
	{
			print_text(2,y++,"Move Slots: " + IntToStr(shell->move_slots.size()) + "/" +IntToStr(shell->max_number_move_slots) );
			y++;
			for (m=shell->move_slots.begin(),_m=shell->move_slots.end();m!=_m;m++)
			{
				temp2 = (CItem *) *m;
			...
			y++;
			print_text(5,y++,"(Speed: " + IntToStr(get_robot_speed()) + ")");
	}
	else
		print_text(2,y++,"- Stationary -");
...
bool CRobot::attack_with_ranged_weapon()
{
...
bool CRobot::shoot_into(int x, int y)
{
  int accuracy;
  int target_x, target_y;
  int random_value;
  ...
  for (m=shell->action_slots.begin(),_m=shell->action_slots.end();m!=_m;m++)
  {
	  ...
	  {
		  CRangedWeapon *sound;
		  if (level.is_player_on_level && (this->seen_now || level.ID=="ben"))
			sound =(CRangedWeapon *) definitions.find_item(range_weapon->name);
		  else
			sound=NULL;
		  switch(range_weapon->fire_type)
...
int CRobot::get_h2h_attack_value()
{
	int attack;
	if (weapon==NULL)
		return 0;
...
int CRobot::get_h2h_defense_value()
{
	int defense;
...
int CIntelligent::turn_robot_on(CItem *to_turn_on)
{
	CRobotShell *shell;
	...
	{
		CIntelligent *intel = shell->owner->IsIntelligent();
		assert(intel!=NULL);
...
bool CRobot::tries_to_turn_off(CMonster *temp)
{
	int chance=0;
	...
	{		
		if (temp->seen_now)
			screen.console.add(temp->name + " has turned " + this->name + " off.",7);
		turn_robot_off();
	...
	{
		if (temp->seen_now)
			screen.console.add(temp->name + " has failed to turn " + this->name + " off.",7);		
	}
	
	return false;
}

bool CMonster::look_around()
{
...
EAction CMadDoctor::get_action() // AI MadDoctor'a
{
...
int CMadDoctor::how_danger_is_cell(int x,int y)
{
	// afraid only of fire
...
t_time CMadDoctor::do_action(EAction action)
{
	last_pX = pX();
	last_pY = pY();
	...
	case ACTION_SHOUT:
		...
		screen.console.add(name + " yells: \"" + to_yell + "\"",15);
...
bool CMadDoctor::death()
{
	seen_last_time_in_turn = level.turn; // zawsze exp. za niego
	level.create_item(pX(),pY(),"Genetic Machine Card",0,0);
	return CMonster::death();	
}

int CMadDoctor::throw_enemy_at_the_wall()
{
	// 1. pick up of enemy
	// 2. throw enemy
  int target_x, target_y;
  ...
  struct SScreen_copy scr_copy;  
     ...
     {
		 if (enemy->seen_now)
			screen.console.add(name + " grabs " + enemy->name + " and throws.",3);
		 else
			 screen.console.add(name + " grabs something and throws.",3);		 
     }
	...
	angle_miss=random(360);
		  ...
		  if (monster!=enemy && monster!=this)
		  {
			  if (monster!=NULL && monster->evade_missile(this)==false) // nie uniknal
			  {
				  if (damage>0)
				  {
					  ...
					  {
						  if (enemy->seen_now)
							  screen.console.add(enemy->name + " hits " + monster->name + " for " + IntToStr(damage) + " damage.",6);				 
						  else
							  screen.console.add("Something hits " + monster->name,6);				 
	 ...
	 if (damage>0)
	 {
		 ...
		 if (enemy!=NULL && enemy->seen_now)
			 screen.console.add(enemy->name + " hits the wall for " + IntToStr(damage) + " damage.",6);
	 }
...
int CMadDoctor::spit_poison()
{
	if (enemy==NULL)
		return false;

	CItem *item;
	item = level.create_item(pX(),pY(),"Sticky Spittle",0,0);
	throw_item_to(enemy->pX(),enemy->pY(),item);
	return true;
}

int CMadDoctor::fire_needle()
{
	if (enemy==NULL)
		return false;
	
	CItem *item;
	item = level.create_item(pX(),pY(),"Needle",0,0);
	throw_item_to(enemy->pX(),enemy->pY(),item);
	return true;
}
		...
		if (monster==NULL) // move all next segments
		{
			...
			while(segment!=NULL)
			{
				...
				if (segment->next==NULL)
				{
					...
					{
						CWorm *new_segment = (CWorm *) level.create_monster(tx,ty,segment->segment_name,0,0);
						if (new_segment!=NULL)
						{
					...
					{
						CWorm *new_tail = (CWorm *) level.create_monster(tx,ty,segment->tail_name,0,0);
						if (new_tail!=NULL)
						{
...
void CWorm::reverse_worm()
{
	// reverse worm
	CWorm *segment=this;
	CWorm *following, *temp;
	while(segment!=NULL)
	{
