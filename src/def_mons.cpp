...
CHero * CLevel :: create_player(int x, int y, string name, int param1, int param2)
{
  CHero *new_one;
  ...
  intel=(CIntelligent *) from_definition;
  
  new_one=new CHero;

  CItem *old_armor= intel->armor;
  if (old_armor!=&intel->no_armor)
  {
	  intel->remove_armor();	   
  }
  
  *((CIntelligent *)new_one)=*intel;
  if (old_armor!=&intel->no_armor)
  {
...
CMonster * CLevel :: create_monster(int x, int y, string name, int param1, int param2)
{
   CMonster *new_one;
   CMonster *from_definition;
