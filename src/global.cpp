...
// global screen
extern struct SScreen_copy screen_copy; // z system.cpp
...
void CMyScreen :: relase()
{
 end_graph();
}

void CMyScreen :: draw_smoke()
{
...
void CMyScreen :: draw2()
{
  draw_all();
  myrefresh();
}

void CMyScreen :: clear_all()
{
  myclear();
}

void CMyScreen :: clear_no_console() // clear bottom of the screen
{
   set_color(0);
   char *pusty="                                                                                ";
...
void CMyScreen :: CConsole :: add_and_zero(string text, int color)
{
   clean();
   add(text,color);
   show();
...
void CMyScreen :: CConsole :: add(string text, int color, bool stop_repeating)
{
	if (text.size()==0)
		return;
...
void CMyScreen :: CConsole :: show_message_buffer(bool show_only_last)
{
		  last_text="";
...
void CMyScreen :: CConsole :: zero()
{
   buffer="";
}


void CMyScreen :: CConsole :: show()
{
   char next_color;
   string word;
   string tmp;
...
int CMyScreen :: draw_box(int color,int left,int top, int right,int down)
{
   int x,y;
   set_color(color);
...
void CMyScreen :: draw_targeting_line(int x1,int y1,int x2, int y2)
{
     int cells;
     unsigned long time_counter;
     int step;
     
     CPosition position[100]; // longest line - 100
...
bool CMyScreen :: draw_stunning_explosion(int *data)
{
	int x,y;
...
bool CMyScreen :: draw_emp_explosion(int *data, int max)
{
	bool player_saw_explosion;
	int x,y;
	set_color(11);
...
bool CMyScreen :: draw_explosion(int *data, int max)
{
	int x,y,a;
...
string CDescriptions::get_description(string to_find)
{
...
string CDescriptions::get_attack(string to_find)
{
	map < string, string >::iterator iter;
...
bool CDescriptions::add_description(string object,string to_add)
{
	descript_texts[object] = to_add;
	return true;
}

bool CDescriptions::add_attack(string object,string to_add)
{
	attacks[object] = attacks[object] + to_add;
	return true;
}

bool CDescriptions::zero_attack(string object)
{
	map < string, string >::iterator iter;
...
bool CDescriptions::show_description(string to_find)
{
	string temp;
	if (to_find=="")
		return false;
